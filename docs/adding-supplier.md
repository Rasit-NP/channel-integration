...
- **본문이 `null` 로 와도 값 없이 끝나지 않는가** (`block()` 이 `null` 을 주면 계약 위반이다)
...
표준 모델 타입들은 생성 시점에 스스로를 검증하므로(`Money` 는 통화 3자리와 음수, `DailyInventory`
는 음수 재고, `SupplierRoomType` 은 수용 인원 1 미만) **공급사가 이상한 값을 하나 보내면 그
생성자가 예외를 던진다.** 안 잡으면 `onErrorResume` 이 받아서 그 묶음이 통째로 실패가 된다.

두 겹으로 막는다.

| | 무엇을 | 어떻게 |
| --- | --- | --- |
| 아는 무효값 | 수용 인원 1 미만, 음수 재고 | 루프에서 명시적으로 걸러 **그 객실 타입·그 날짜만** 뺀다 |
| 나머지 | 통화·금액 등 예상 못 한 값 | `toOffers` / `toProperties` 의 항목 경계에서 `IllegalArgumentException` 을 잡아 **그 항목만** 뺀다 |

앞은 손실을 최소로 하고(숙소 하나를 통째로 버리지 않는다), 뒤는 예상 못 한 값에도 포트 계약이
지켜지게 한다. `NullPointerException` 은 잡지 않는다 — 그건 공급사 데이터 문제가 아니라 매퍼가
null 검사를 빠뜨린 우리 쪽 버그다.

**④ 본문이 `null` 로 오면 값 없이 끝난다.**
`bodyToMono` 는 **null 로 디코딩되는 본문**에서 아무것도 내보내지 않아 `map` 이 불리지 않는다.
그대로 두면 "어떤 경우에도 결과를 돌려준다"는 포트 계약이 그 경우에 깨지고, 동기화 보고서에서는
그 공급사가 **아예 사라진다.** `defaultIfEmpty` 로 막는다.

세 가지를 구분해야 한다 — 빈 문자열은 디코더가 오류로 잡아 `MALFORMED_RESPONSE` 가 되고,
`{}` 는 정상적인 빈 결과이며, **`null` 만 값 없이 끝난다.**
